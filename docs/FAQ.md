...
For v1.7.5 verification, it successfully opened and parsed all 15 CAKs from three WWE 2K26 installations (original-release, v1.14, and v1.16), including three substantially different `bakedfile100.cak` versions.
...
It does not force-install on unknown executable hashes. Support is limited to explicitly reviewed profiles; “all WWE 2K26 versions” is a roadmap goal, not a v1.7.5 claim. In this release, the allowlist includes only the validated WWE 2K26 v1.14 executable hash `7f558607bd6881c4547d752ea107fb03282a2980e47ef19ade597e096de4a0e9`. Other newly observed hashes are intentionally blocked until separately validated.

## How are hotfix versions labeled?

If hotfixes are needed after a release, the convention is to append a letter suffix instead of skipping to a new feature version number (for example: `v1.7.5a`, `v1.7.5b`, …). The same A–Z suffix convention is used for other version tracks (for example: `v1.0.1a` for Secure DataCtrlLink and `v0.1.01a` for the 2K19 project research preview).
