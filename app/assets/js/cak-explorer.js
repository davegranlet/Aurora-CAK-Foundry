(function () {
  ...
  byId('repackChooseSource').addEventListener('click', async () => { try { const result = await api.chooseRepackSource(); if (!result.ok) return; state.repackSource = result.path; byId('repackSourcePath').textContent = result.path; byId('repackBuild').disabled = false; byId('repackVerify').disabled = true; byId('repackOpenOutput').disabled = true; message('repackMessage', `BakeMe folder verified: ${result.validation.fileCount.toLocaleString()} mod file(s) under ${result.validation.recognisedRoots.join(', ')}.`, 'good'); } catch (error) { message('repackMessage', error.message, 'bad'); } });
