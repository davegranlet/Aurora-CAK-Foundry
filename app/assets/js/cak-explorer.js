(function () {
  ...
  byId('repackAuditSource').addEventListener('click', async () => { try { const result = await api.auditRepackSource(); if (!result.ok) return; state.repackAudit = result; byId('repackPrepareCopy').disabled = !result.suggestedRoot; const size = `${(result.bytes / (1024 * 1024)).toFixed(1)} MiB`; const suggestion = result.suggestedRoot ? ` Suggested BakeMe root: ${result.suggestedRoot}` : ''; const candidates = result.candidates.length ? ` Found ${result.candidates.length} possible game-root folder${result.candidates.length === 1 ? '' : 's'}.` : ''; const warnings = result.warnings.length ? ` ${result.warnings.join(' ')}` : ''; byId('repackAuditPath').textContent = result.root; message('repackAuditMessage', `Inspected ${result.files.toLocaleString()} file(s), ${size}, through ${result.maxDepth} folder levels.${candidates}${suggestion}${warnings}`, result.warnings.length ? 'working' : 'good'); } catch (error) { message('repackAuditMessage', error.message, 'bad'); } });
  byId('repackPrepareCopy').addEventListener('click', async () => { if (!state.repackAudit || !state.repackAudit.suggestedRoot) return; try { const result = await api.prepareBakeMeCopy(state.repackAudit.root); if (!result.ok) return; state.repackSource = result.destination; byId('repackSourcePath').textContent = result.destination; byId('repackBuild').disabled = false; byId('repackVerify').disabled = true; byId('repackOpenOutput').disabled = true; const blocked = result.blocked.length ? ` ${result.blocked.length} item(s) were not copied because they exceeded the depth limit or were links.` : ''; message('repackMessage', `Clean BakeMe copy created: ${result.copiedFiles.toLocaleString()} file(s) from ${result.sourceRoot}.${blocked}`, result.blocked.length ? 'working' : 'good'); } catch (error) { message('repackMessage', error.message, 'bad'); } });
